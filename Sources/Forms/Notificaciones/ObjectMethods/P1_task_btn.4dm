...
If ($b_mostrarEnExplorador)
	$t_popupItems:=$t_popupItems+"Mostrar en el explorador;"
Else 
	$t_popupItems:=$t_popupItems+"(Mostrar en el explorador;"
End if 
...
Case of 
	: ($l_userChoice=1)
		<>vtMSG_msgUUID:=vt_UUIDmensaje
		
		$l_moduleProcess:=Process number:C372("Explorador "+[NTC_Notificaciones:190]Explorador_modulo:13)
		If ($l_moduleProcess>0)
			IP_SendMessage ($l_moduleProcess;"NTC_MuestraDatosEnExplorador";[NTC_Notificaciones:190]Auto_UUID:1)
