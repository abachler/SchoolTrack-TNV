  ...
  // Por: Alberto Bachler K.: 10-11-14, 08:35:22
  //  ---------------------------------------------
  //
  //
  //  ---------------------------------------------
...
Case of 
	: (Form event:C388=On Load:K2:1)
		...
		For ($i;1;Count list items:C380($l_volumes))
			...
			End if 
			SET LIST ITEM ICON:C950(hl_volumes;vl_refIndex;$vp_Icon)
			$blob:=SYS_GetServerFolderHList ($volumeName)
			If (BLOB size:C605($blob)>0)
				...
				For ($ii;1;Count list items:C380($subList))
					...
					$vp_Icon:=SYS_GetServerDocumentIcon ($folderPath;16)
