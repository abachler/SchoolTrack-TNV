//%attributes = {}
  // MÉTODO: EV2_Calculos_ConsolidaExamen
  // ----------------------------------------------------
  // Usuario (OS): Alberto Bachler
  // Fecha de creación: 25/03/11, 07:23:00
  // ----------------------------------------------------
  // DESCRIPCIÓN
  // 
  //
...
If (OK=1)
	...
	[Alumnos_Calificaciones:208]ExamenAnual_Nota:17:=-10
	[Alumnos_Calificaciones:208]ExamenAnual_Puntos:18:=-10
	[Alumnos_Calificaciones:208]ExamenAnual_Simbolo:19:=""
	...
	For ($iParciales;1;Size of array:C274($alAS_EvalPropSourceID))
		...
		Case of 
			: ($alAS_EvalPropSourceID{$iParciales}>0)  //asignaturas hijas
				...
				If (OK=1)
					$evaluacion:=[Alumnos_Calificaciones:208]ExamenAnual_Real:16
					Case of 
						: ($evaluacion=-3)
	...
	Case of 
		...
		: (($conPonderaciones) & ($evaluaciones>0))
			[Alumnos_Calificaciones:208]ExamenAnual_Real:16:=Round:C94($sum/$ponderacionesAcumuladas*100;11)
			
		: (($conCoeficientes) & ($evaluaciones>0))
			[Alumnos_Calificaciones:208]ExamenAnual_Real:16:=Round:C94($sum/$div;11)
			
		: (($sum>0) & ($div>0))
			[Alumnos_Calificaciones:208]ExamenAnual_Real:16:=Round:C94($sum/$div;11)
	End case 
	
	If ((vi_TruncarInferiorRequerido=1) & ([Alumnos_Calificaciones:208]ExamenAnual_Real:16<rPctMinimum))
		Case of 
			: (iEvaluationMode=Notas)
				$r_Nota:=EV2_Real_a_Nota ([Alumnos_Calificaciones:208]ExamenAnual_Real:16;vi_TruncarInferiorRequerido;iGradesDEC)
				[Alumnos_Calificaciones:208]ExamenAnual_Real:16:=EV2_Nota_a_Real ($r_Nota)
			: (iEvaluationMode=Puntos)
				$r_Puntos:=EV2_Real_a_Puntos ([Alumnos_Calificaciones:208]ExamenAnual_Real:16;vi_TruncarInferiorRequerido;iPointsDEC)
				[Alumnos_Calificaciones:208]ExamenAnual_Real:16:=EV2_Puntos_a_Real ($r_Puntos)
		End case 
		
	End if 
	
	
	If ([Alumnos_Calificaciones:208]ExamenAnual_Real:16>=vrNTA_MinimoEscalaReferencia)
