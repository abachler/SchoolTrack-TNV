...
For ($day;1;31)  // repetir 31 veces  
	$date:=DT_GetDateFromDayMonthYear ($day;$month;<>gYear)
	If (DateIsValid ($date;0))
		$dias:=$dias+1
	End if 
End for 

$text:="Asistencia para el mes de "+<>atXS_MonthNames{$month}+" "+String:C10(<>gYear)+"\r"+"Dias habiles:"+"\t"+String:C10($dias)+("\r"*3)
...
$dias:=0
$l_ProgressProcID:=IT_Progress (1;0;$l_ProgressProcID;__ ("Generando tabla de inasistencia..."))
For ($day;1;31)  // repetir 31 veces  
	$date:=DT_GetDateFromDayMonthYear ($day;$month;<>gYear)
	...
	Else 
		...
		SEND PACKET:C103($ref;$text)
	End if 
	$l_ProgressProcID:=IT_Progress (0;$l_ProgressProcID;$day/31)
End for 
