//%attributes = {}
  // Método: KRL_VerifyAutomaticRelations
  //----------------------------------------------
	...
	If (Is table number valid:C999($iTables))
		...
		For ($iFields;1;Get last field number:C255($iTables))
			If (Is field number valid:C1000($iTables;$iFields))
				...
				If (($autoOne) & ($oneTable#0) & ($oneField#0))
...
$l_ProgressProcID:=IT_Progress (1;0;$l_ProgressProcID;"Verificando relaciones automáticas...")
For ($iRelations;1;Size of array:C274($aRelationsFrom))
	...
	$l_ProgressProcID:=IT_Progress (0;$l_ProgressProcID;$iRelations/Size of array:C274($aRelationsFrom);"Verificando relaciones automáticas...\r"+$relacion)
