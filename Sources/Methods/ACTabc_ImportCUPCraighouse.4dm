...
While ($text#"")
	...
	aMonto{Size of array:C274(aMonto)}:=Num:C11(Substring:C12($text;101;12))
	aCodFamilia{Size of array:C274(aCodFamilia)}:="0"
	aRUT{Size of array:C274(aRUT)}:=ST_DeleteCharsLeft (Substring:C12($text;30;10);"0")
	...
	$vt_fecha:=Substring:C12($text;22;8)
	$vd_fecha:=DT_GetDateFromDayMonthYear (Num:C11(Substring:C12($vt_fecha;1;2));Num:C11(Substring:C12($vt_fecha;3;2));Num:C11(Substring:C12($vt_fecha;5;4)))
	$valorUF:=ACTut_fValorUF ($vd_fecha)
	$vr_montoMonOriginal:=Num:C11(Substring:C12($text;81;12)+<>tXS_RS_DecimalSeparator+Substring:C12($text;93;2))
	...
	End if 
	  //aMontoMora{Size of array(aMontoMora)}:=Num(Substring($text;113;12))
	RECEIVE PACKET:C104($ref;$text;$delimiter)
	$text:=_O_Win to Mac:C464($text)
End while 
