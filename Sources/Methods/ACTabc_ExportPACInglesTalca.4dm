...
For ($i;1;Size of array:C274(aidsAvisos))
	...
	If ($linea=-1)
		...
		at_CodConv{1}:="001"
		as_TipoReg{1}:="D"
		at_IdServ{1}:=ST_Uppercase ([Personas:7]ACT_RUTTitutal_Cta:50)
		ar_Monto{1}:=$FieldPtr->
		...
		at_FechaVenc{1}:=Substring:C12(String:C10(vl_AñoApdo;"0000");3;2)+String:C10(vl_MesApdo;"00")+$diaFecha
	Else 
...
$montoTotal:=ST_RigthChars (("0"*12)+String:C10(AT_GetSumArray (->ar_Monto));12)
$numTrans:=ST_RigthChars (("0"*10)+String:C10(Size of array:C274(ar_Monto));10)
$footer:="09000100000889498005001"+at_FechaVenc{1}+$numTrans+$montoTotal+"\r"
IO_SendPacket ($ref;$footer)

For ($i;1;Size of array:C274(ar_Monto))
	$line:=at_CodBanc{$i}+ST_RigthChars (("0"*10)+at_IdServ{$i};10)+(" "*12)+ST_RigthChars (("0"*15)+at_IdServ{$i};15)+at_FechaVenc{$i}+ST_RigthChars (("0"*10)+String:C10(ar_Monto{$i});10)+"\r"
