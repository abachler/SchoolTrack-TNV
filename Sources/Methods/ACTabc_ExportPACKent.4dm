...
For ($i;1;Size of array:C274(aidsAvisos))
	...
	If ($linea=-1)
		...
		aServicio{1}:=ST_LeftChars ([Familia:78]Codigo_interno:14+(" "*22);22)
		aRUTPAC{1}:=ST_Uppercase (ST_RigthChars ((" "*15)+"19";15))
...
$montoTotal:=ST_RigthChars ("000000000000"+String:C10(AT_GetSumArray (->aMonto));12)  //Colegio indica que los montos van sin decimales. Se modifica línea. Ver incidente  52729
$header:="0900010000"+"0898781003"+"001"+$fecha+$numTrans+$montoTotal+"\r"
...
For ($i;1;Size of array:C274(aCodigo))
	...
	$line:=$line+ST_RigthChars ("0000000000"+String:C10(aMonto{$i});10)+"\r"  //Colegio indica que los montos van sin decimales. Se modifica línea. Ver incidente  52729
