...
While ($text#"")
	...
	aMonto{Size of array:C274(aMonto)}:=Num:C11(ST_GetWord ($text;4;";"))
	aNumTarjeta{Size of array:C274(aNumTarjeta)}:=ST_GetCleanString (ST_GetWord ($text;1;";"))
	aNombre{Size of array:C274(aNombre)}:=ST_GetCleanString (ST_GetWord ($text;2;";"))
	aRUT{Size of array:C274(aRUT)}:=Replace string:C233(ST_GetCleanString (ST_GetWord ($text;3;";"));"-";"")
	aCodAprobacion{Size of array:C274(aCodAprobacion)}:=ST_GetCleanString (ST_GetWord ($text;5;";"))
	aDescCodigo{Size of array:C274(aDescCodigo)}:=ST_CleanString (ST_GetWord ($text;6;";"))
	RECEIVE PACKET:C104($ref;$text;$delimiter)
	$text:=_O_Win to Mac:C464($text)
End while 
