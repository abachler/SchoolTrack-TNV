...
For ($i;1;Size of array:C274(aidsAvisos))
	...
	If ($linea=-1)
		AT_Insert (1;1;->aIndicador;->aMonto;->aNumTarjeta;->aNombre;->aRUT;->aServicio;->aVencTarjeta;->aidsPersonas)
		...
		aServicio{1}:=Substring:C12(aRUT{1};1;22)
	Else 
...
For ($i;1;Size of array:C274(aIndicador))
	$line:=aIndicador{$i}+";"+String:C10(aMonto{$i})+";"+aNumTarjeta{$i}+";"+aVencTarjeta{$i}+";"+aNombre{$i}+";"+aRUT{$i}+";"+aServicio{$i}+"\r"
...
End for 
$line:=";"+String:C10($total)+";;;;;"
IO_SendPacket ($ref;$line)
$l_ProgressProcID:=IT_Progress (-1;$l_ProgressProcID)
vnumTransPAT:=String:C10(Size of array:C274(aIndicador))
CLOSE DOCUMENT:C267($ref)

AT_Initialize (->aIndicador;->aMonto;->aNumTarjeta;->aNombre;->aRUT;->aServicio;->aVencTarjeta;->aidsAvisos;->aidsPersonas)