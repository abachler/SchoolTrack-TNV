...
If (vl_AplicaEnNiveles ?? $l_numeroNivelEnLista)  // (la variable vl_AplicaEnNiveles tiene asignado el valor del campo [MPA_DefinicionEjes]BitsNiveles)
	  // El Eje aplica actualmente en el nivel seleccionado...
	
	  // busco los enunciados (ejes y dimensiones y competencias asociadas) utilizados en matrices y los pongo en un conjunto
	...
	$l_objetos:=Records in set:C195("$Objetos")
	  ...
	  // busco las dimensiones asociadas al eje que aplixan en el nivel y las pongo en un conjunto
	QUERY:C277([MPA_DefinicionDimensiones:188];[MPA_DefinicionDimensiones:188]ID_Eje:3=[MPA_DefinicionEjes:185]ID:1)
	QUERY SELECTION BY FORMULA:C207([MPA_DefinicionDimensiones:188];[MPA_DefinicionDimensiones:188]BitsNiveles:21 ?? $l_numeroNivelEnLista)
	CREATE SET:C116([MPA_DefinicionDimensiones:188];"$Dimensiones")
	
	  // busco las competencias asociadas al eje y las pongo en un conjunto
	...
	CREATE SET:C116([MPA_DefinicionCompetencias:187];"$Competencias")
	  ...
	  // busco los registros de evaluación de aprendizajes (ejes y dimensiones y competencias asociadas) correspondientes al eje
	...
	$l_resultado:=1
	vt_RetiroNivel:="Inhabilitar Eje de aprendizaje en "+<>at_NombreNivelesActivos{<>al_NumeroNivelesActivos}
	...
	If ($l_transaccionOK=1)
		...
		For ($iDimensiones;1;Size of array:C274($al_recNumObjetos))
			READ WRITE:C146([MPA_DefinicionDimensiones:188])
			KRL_GotoRecord (->[MPA_DefinicionDimensiones:188];$al_recNumObjetos{$iDimensiones};True:C214)
			If (OK=1)
				  // apago en las dimensiones los bits que no están encendidos en el Eje del que depende
				For ($iNiveles;1;24)
					$l_nivelNumero:=<>aNivNo{$iNiveles}
					If (([MPA_DefinicionDimensiones:188]BitsNiveles:21 ?? $iNiveles) & (Not:C34(vl_AplicaEnNiveles ?? $iNiveles)))
						[MPA_DefinicionDimensiones:188]BitsNiveles:21:=[MPA_DefinicionDimensiones:188]BitsNiveles:21 ?- $iNiveles
					End if 
				End for 
				
				  // si la dimensión estaba habilitada en una etapa que incluía el nivel que se inhabilita
				  // desasigno la etapa y especifico que la dimensión está asociada a niveles, no a una etapa
				For ($i;[MPA_DefinicionDimensiones:188]DesdeGrado:6;[MPA_DefinicionDimensiones:188]HastaGrado:7)
					$l_indexNivel:=Find in array:C230(<>aNivNo;$i)
					If (Not:C34([MPA_DefinicionDimensiones:188]BitsNiveles:21 ?? $l_indexNivel))
			...
			Else 
				$iDimensiones:=Size of array:C274($al_recNumObjetos)
				$l_transaccionOK:=0
			End if 
		End for 
	End if 
	...
	If ($l_transaccionOK=1)
		...
		For ($iCompetencias;1;Size of array:C274($al_recNumObjetos))
			...
			If (OK=1)
				  // apago en las competencias los bits que no están encendidos en el Eje del que depende
				For ($iNiveles;1;24)
	...
	If ($l_transaccionOK=1)
		...
		If (Not:C34(Is new record:C668([MPA_DefinicionEjes:185])))
...
Else 
	...
	End for 
	If ($l_EtapaNivel<=0)
		$t_mensaje:=__ ("El área no esta configurada para ser utilizada en ^0.\r\rNo es posible habilitar el Eje de aprendizaje en este nivel.")
