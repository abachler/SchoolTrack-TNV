//%attributes = {}
  // Método: ACTabc_ImportPATYaocalli3
  //----------------------------------------------
  // Usuario (OS): roberto
  // Fecha: 08-04-10, 12:45:38
  // ---------------------------------------------
  // Descripción: 
  // 
  // Parámetros:
...
While ($text#"")
	...
	aMonto{Size of array:C274(aMonto)}:=Num:C11(Substring:C12($text;78;8)+<>tXS_RS_DecimalSeparator+Substring:C12($text;86;2))
	aRUT{Size of array:C274(aRUT)}:=ST_DeleteCharsLeft (Substring:C12($text;110;6);"0")
	...
	$vt_referencia:=Substring:C12($text;110;12)
	$vl_noCuota:=Num:C11(Substring:C12($text;118;2))
	$vl_ciclo:=Num:C11(Substring:C12($text;120;2))
	...
	End if 
	aFechaPagos{Size of array:C274(aFechaPagos)}:=DT_GetDateFromDayMonthYear (Num:C11(Substring:C12($text;17;2));Num:C11(Substring:C12($text;19;2));Num:C11(Substring:C12($text;21;4)))
	$vlACT_concepto:=Num:C11(Substring:C12($text;116;2))
					...
					If (Records in selection:C76([ACT_Cargos:173])>0)
						...
						If (Records in selection:C76([ACT_Avisos_de_Cobranza:124])=1)
							If (Abs:C99(ACTcar_CalculaMontos ("calcMontoFromNumAvisoMEmision";->[ACT_Avisos_de_Cobranza:124]ID_Aviso:1;->[ACT_Cargos:173]Saldo:23;Current date:C33(*)))=[ACT_Avisos_de_Cobranza:124]Monto_Neto:11)
								al_idAvisoAPagar{Size of array:C274(al_idAvisoAPagar)}:=[ACT_Avisos_de_Cobranza:124]ID_Aviso:1
							Else 
								aDescCodigo{Size of array:C274(aDescCodigo)}:="EL AVISO QUE DESEA PAGAR ESTA PAGADO PARCIALMENTE O TOTALMENTE. Aviso de cobranza"+" paga"+"do para la referencia "+$vt_referencia+", para cargos pertenecientes a la categoría inscripción."
								aCodAprobacion{Size of array:C274(aCodAprobacion)}:="-1"
							End if 
						Else 
				...
				If (Records in selection:C76([ACT_Cargos:173])>0)
					...
					If (Records in selection:C76([ACT_Avisos_de_Cobranza:124])=1)
						If (Abs:C99(ACTcar_CalculaMontos ("calcMontoFromNumAvisoMEmision";->[ACT_Avisos_de_Cobranza:124]ID_Aviso:1;->[ACT_Cargos:173]Saldo:23;Current date:C33(*)))=[ACT_Avisos_de_Cobranza:124]Monto_Neto:11)
							al_idAvisoAPagar{Size of array:C274(al_idAvisoAPagar)}:=[ACT_Avisos_de_Cobranza:124]ID_Aviso:1
						Else 
							aDescCodigo{Size of array:C274(aDescCodigo)}:="EL AVISO QUE DESEA PAGAR ESTA PAGADO PARCIALMENTE O TOTALMENTE. Aviso de cobranza"+" paga"+"do para la referencia "+$vt_referencia+", para cargos llamados "+ST_Qte ("UNAM")+"."
							aCodAprobacion{Size of array:C274(aCodAprobacion)}:="-1"
						End if 
					Else 
						aDescCodigo{Size of array:C274(aDescCodigo)}:="EXISTE MÁS DE UN AVISO DE COBRANZA EMITIDO PARA EL MES QUE DESEAS PAGAR. Aviso de"+" cobranza no encontrado para la referencia "+$vt_referencia+", para cargos llamados "+ST_Qte ("UNAM")+"."
						aCodAprobacion{Size of array:C274(aCodAprobacion)}:="-1"
					End if 
				Else 
			...
			If (aFechaPagos{Size of array:C274(aFechaPagos)}>[ACT_Avisos_de_Cobranza:124]Fecha_Vencimiento:5)
				...
				If (aFechaPagos{Size of array:C274(aFechaPagos)}<=$vd_fechaV)
					...
					If (Records in selection:C76([ACT_Cargos:173])>1)
						...
						If ((Sum:C1([ACT_Cargos:173]MontosPagados:8)=0) & ($vl_records=0) & ($vl_records2=0))
