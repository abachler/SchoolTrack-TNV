...
$deuda:=ACTpgs_BuscaDeuda ([Personas:7]No:1;$vl_idAvisoAPagar;$vdACT_FechaUF;$vl_idItemPagar;$vt_familia)
...
Else 
	If ($display)
		...
		Case of 
			: (vrACTpgs2_deuda#0)
				CD_Dlog (0;__ ("El apoderado ")+vsACT_NomApellido+__ (" tiene saldo a favor de ")+String:C10($deuda;$format)+__ (" y a la vez tiene cargos con saldo.\r\rBusque los pagos con Disponible y use la opción "+ST_Qte ("Utilizar Disponible")+", que aparece en el popup lateral de la pestaña Pagos."))
			: ($deuda=0)
				CD_Dlog (0;__ ("El apoderado ")+vsACT_NomApellido+__ (" no tiene deuda."))
			Else 
				CD_Dlog (0;__ ("El apoderado ")+vsACT_NomApellido+__ (" tiene un saldo a favor de ")+String:C10($deuda;$format)+__ ("."))
		End case 
	End if 
...
BRING TO FRONT:C326(Current process:C322)
  ...
  //20130405 RCH No era posible ingresar pagos a apoderados sin deuda...
  //IT_SetButtonState ($deuda<0;->bIngresarPago)
