...
TBL_ShowChoiceList (0;"Seleccione la Cuenta";2;->vtACT_CPCAFecha)
If (ok=1)
	If ((vtACT_CPCAFecha#<>asACT_CuentaCta{choiceIdx}) | (vtACT_CAUXCCAFecha#<>asACT_CodAuxCta{choiceIdx}) | (Windows Ctrl down:C562 | Macintosh command down:C546))
		vtACT_CPCAFecha:=<>asACT_CuentaCta{choiceIdx}
		vtACT_CAUXCCAFecha:=<>asACT_CodAuxCta{choiceIdx}
		...
		If (ok=1)
			If (r2=1)
				...
				AT_Populate (->asNCta;->vtACT_CPCAFecha)
				AT_Populate (->asCACta;->vtACT_CAUXCCAFecha)
		...
		Else 
			vtACT_CPCAFecha:=vtACT_CPCAFechaCurrState
			vtACT_CAUXCCAFecha:=vtACT_CAUXCCAFechaCurrState
		End if 
	End if 
End if 