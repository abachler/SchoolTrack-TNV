  // [xxSTR_EstilosEvaluacion].Configuration.escalaNotas_decimalesPeriodicas()
  //
  //
  // creado por: Alberto Bachler Klein: 11-07-16, 12:25:48
  // -----------------------------------------------------------
...
Case of 
	: (Form event:C388=On Mouse Enter:K2:33)
		...
		$t_mensaje:=__ ("Número de decimales aceptados durante el registro de calificaciones en la escala de notas.")
		If ($l_evaluaciones>0)
	...
	: (Form event:C388=On Data Change:K2:15)
		...
		$l_digitosMaximoEscala:=Length:C16(String:C10(rGradesTo))
		...
		Else 
			...
			If ($l_decimales>0)
				...
				If (Length:C16($t_cadenaDecimales)>$l_decimales)
					...
					If ($r_intervalo=0)
						$t_formatoDecimales:="0"*$l_decimales
						$r_intervalo:=Num:C11("0"+<>tXS_RS_DecimalSeparator+String:C10(1;$t_formatoDecimales))
					End if 
					BEEP:C151
				Else 
					$t_formatoDecimales:="0"*$l_decimales
