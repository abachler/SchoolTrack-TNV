	...
	Case of 
		: (Records in selection:C76([Personas:7])=0)
			...
			If ($r=1)
				...
				End if 
				WDW_Open ($width;$height;2;4;XSvs_nombreTablaLocal_puntero (yBWR_currentTable);"wdw_CloseDlog")
				ADD RECORD:C56([Personas:7];*)
				CLOSE WINDOW:C154
				
				If ($vl_recNumAl>=0)
					KRL_GotoRecord (->[Alumnos:2];$vl_recNumAl;True:C214)
				End if 
				
				If ($vl_RecNumCta>=0)
					KRL_GotoRecord (->[ACT_CuentasCorrientes:175];$vl_RecNumCta;True:C214)
				End if 
				...
				vyBWR_CustomArrayPointer:=$vyBWR_CustomArrayPointer
				  ...
				  //ticket  148957 JVP 
				GOTO RECORD:C242([Alumnos:2];$vl_recNumAl)
				  //fin codigo
