...
Case of 
	...
	: (Form event:C388=On Getting Focus:K2:7)
		...
		tempSelf:=Self:C308->
		tempID:=[ACT_Terceros:138]PAC_Banco_ID:45
	: (Form event:C388=On Losing Focus:K2:8)
		...
		Else 
			...
			[ACT_Terceros:138]PAC_Banco_ID:45:=atACT_BankID{$page}
		End if 
		If ($page#-1)
			If ($valuebank#"")
				Self:C308->:=$valuebank
				[ACT_Terceros:138]PAC_Banco_ID:45:=$valueID
			End if 
			  //GOTO AREA(vtACT_BancoCuenta)
		Else 
			BEEP:C151
			Self:C308->:=tempSelf
			[ACT_Terceros:138]PAC_Banco_ID:45:=tempID
		End if 
End case 