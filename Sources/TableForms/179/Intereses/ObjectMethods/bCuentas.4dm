...
TBL_ShowChoiceList (0;"Seleccione la Cuenta";2;->vsACT_CtaIntereses)
If (ok=1)
	If ((vsACT_CtaIntereses#<>asACT_CuentaCta{choiceIdx}) | (vsACT_CodIntereses#<>asACT_CodAuxCta{choiceIdx}) | (Macintosh command down:C546 | Windows Ctrl down:C562))
		vsACT_CtaIntereses:=<>asACT_CuentaCta{choiceIdx}
		vsACT_CodIntereses:=<>asACT_CodAuxCta{choiceIdx}
		vtMsg:="Usted modificó el código de plan de cuentas para los intereses."+"\r"+"Si lo desea puede aplicar el nuevo código a los intereses ya generados o bien uti"+"liz"+"ar este nuevo código sólo para los intereses que se generen de ahora en adelante."
		...
		If (ok=1)
			If (r2=1)
				...
				AT_Populate (->asNCta;->vsACT_CtaIntereses)
				AT_Populate (->asCACta;->vsACT_CodIntereses)
				  //0xDev_AvoidTriggerExecution (True)
				ARRAY TO SELECTION:C261(asNCta;[ACT_Cargos:173]No_de_Cuenta_contable:17;asCACta;[ACT_Cargos:173]CodAuxCta:43)
