...
If (ok=1)
	Case of 
		...
		: (r2=1)
			C_LONGINT:C283($proc)
			$proc:=IT_UThermometer (1;0;__ ("Aplicando cambio a cargos ya generados/emitidos..."))
			READ WRITE:C146([ACT_Cargos:173])
			QUERY:C277([ACT_Cargos:173];[ACT_Cargos:173]Ref_Item:16=vl_idIE)
			APPLY TO SELECTION:C70([ACT_Cargos:173];[ACT_Cargos:173]NoAfecto_a_DescuentosAut:60:=(Self:C308->=1))
			KRL_UnloadReadOnly (->[ACT_Cargos:173])
			IT_UThermometer (-2;$proc)
			LOG_RegisterEvt ("Configuración ítems: Campo "+ST_Qte ($vt_nombreCampo)+" modificado a: "+ST_Boolean2Str ((Self:C308->=1);"Verdadero";"Falso"))
	End case 
End if 