...
If ($parentRef=0)
	...
	$abajo:=0
	For ($i;$pos+1;Count list items:C380(hl_Categorias))
		GET LIST ITEM:C378(hl_Categorias;$i;$tempref;$temptext)
		$parentRef:=List item parent:C633(hl_Categorias;$tempref)
		If ($parentRef=0)
			$abajo:=$i
			$i:=Count list items:C380(hl_Categorias)+1
		End if 
	End for 
	If ($abajo#0)
		GET LIST ITEM:C378(hl_Categorias;$abajo;$itemRef;$itemText)
	...
	READ ONLY:C145([xxACT_Items:179])
	  // Modificado por: Saúl Ponce (21-03-2018) Ticket 198530, para que al bajar una categoría no se vuelva a mostrar "Items sin categoria"
