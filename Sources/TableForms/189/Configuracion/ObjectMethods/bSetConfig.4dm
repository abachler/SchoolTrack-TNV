  ...
  //KRL_GotoRecord (->[MPA_AsignaturasMatrices];$recNum;False)
...
If (vlMPA_IDMatrizActual#[MPA_AsignaturasMatrices:189]ID_Matriz:1)
	...
	Case of 
		: (Records in selection:C76([Alumnos_EvaluacionAprendizajes:203])>0)
			$event:=__ ("La matriz de evaluación fue reemplazada en ^0 por la matriz \"^1\" y las evaluaciones existentes eliminadas previa confirmación del usuario.")
			$event:=Replace string:C233($event;"^0";[Asignaturas:18]denominacion_interna:16+", "+[Asignaturas:18]Curso:5)
			$event:=Replace string:C233($event;"^1";[MPA_AsignaturasMatrices:189]NombreMatriz:2)
			OK:=CD_Dlog (0;__ ("En esta asignatura ya se han registrado evaluaciones.\r\rSi usted asigna la matriz de evaluación de aprendizajes seleccionada esas evaluaciones serán eliminadas.\r\r¿Desea asignar la matriz seleccionada de todas maneras?");__ ("");__ ("No");__ ("Si. Cambiar matriz."))
		: (Records in set:C195("aprendizajes")>0)
			$event:=__ ("La matriz de evaluación fue reemplazada en ^0 por la matriz \"^1\" previa confirmación del usuario.")
			$event:=Replace string:C233($event;"^0";[Asignaturas:18]denominacion_interna:16+", "+[Asignaturas:18]Curso:5)
			$event:=Replace string:C233($event;"^1";[MPA_AsignaturasMatrices:189]NombreMatriz:2)
			OK:=CD_Dlog (0;__ ("Los Aprendizajes definidos en la matriz de evaluación utilizada por la asignatura actualmente serán reemplazados por los de la matriz  \"^0\".\r\r¿Desea reemplazar la matriz actual por la matriz \"^0\"?");__ ("");__ ("No");__ ("Si. Cambiar matriz."))
		Else 
			OK:=2
			$event:=__ ("La matriz de evaluación fue reemplazada en ^0 por la matriz \"^1\".")
			$event:=Replace string:C233($event;"^0";[Asignaturas:18]denominacion_interna:16+", "+[Asignaturas:18]Curso:5)
			$event:=Replace string:C233($event;"^1";[MPA_AsignaturasMatrices:189]NombreMatriz:2)
			OK:=CD_Dlog (0;__ ("Los Aprendizajes definidos en la matriz de evaluación utilizada por la asignatura actualmente serán reemplazados por los de la matriz  \"^0\".\r\r¿Desea reemplazar la matriz actual por la matriz \"^0\"?");__ ("");__ ("No");__ ("Si. Cambiar matriz."))
	End case 
